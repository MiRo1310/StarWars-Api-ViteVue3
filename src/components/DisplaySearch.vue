...
<template>
    <div class="text-right text--black-bgWhite">
        ...
        <button v-if="searchDisplayed && props.searchedText != ''" @click="emit('displaySearch', false)"
            class="button--blackYellow fixed md:right-16 right-4 lg:top-48 md:top-40 top-36 border h-4 w-4 text-xs"
            ...
            <template v-if="searchDisplayed && props.noValueToSearch && props.resultsFound"
                ...
                <li class="font-bold my-2 mx-1">
                    <a href="#" @click="emit('loadInfo', item.url)">
                        <template v-for="item in     Utils.extractSearchFromText(item.name, props.searchedText)">
                        ...
                        <ul>
                            ...
                            <li class="text-xxs">
                                <template v-for="item in     Utils.extractSearchFromText(item.search, props.searchedText)">
                                    <span v-if="item.toLowerCase() != props.searchedText.toLowerCase() && item != ''">{{
                                        item
                                    }}</span>
                                    <span v-else class="search--searchedText">{{ item }}</span>
