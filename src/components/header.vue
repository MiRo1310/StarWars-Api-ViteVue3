...
<template>
    <header class="bg--header  border-b-4 border--header border-double pb-4 fixed w-full pt-0 top-0 p-10 text-center"
        ...
        <h1 class="lg:text-5xl font--primary md:text-3xl sm:text-xl xxs:text-xl text-center md:m-3 inline-block rounded-md">
            <span class="cursor-pointer" v-on:click="loadSide">Star Wars</span>
        </h1>
        <p v-if="pageData.isLoading">Loading...</p>
        <p v-if="pageData.isReloading" class="font--primary animate-fade">Data will be reloaded!</p>
        ...
        <p v-if="!pageData.isStarting"
            ...
            {{ store.records }} {{ stringJs.firstLetterToUpperCase(pageData.actualCategory) }} of the Star Wars Universe</p>
